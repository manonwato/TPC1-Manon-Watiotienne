...
void GaussSeidel_dynamic(double*** a, double** b, double** u_old, double** u_new, double* residual, double hx, double hy, double tol)
{
    ...
    if(it < Niter_max)
        printf("[Gauss Seidel-dynamic] converged after %ld iterations !\n", it);
    else
        printf("[Gauss Seidel-dynamic] maximum iteration reached without converged !\n");
}
